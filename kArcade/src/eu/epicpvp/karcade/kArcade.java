...
public class kArcade extends JavaPlugin {
	...
	public void onEnable() {
		try {
			...
			for (GameType type : GameType.values())
				UtilFile.DeleteFolder(new File(type.getShortName()));
			...
			UtilServer.createClient(this, (id == -1 ? ClientType.OTHER : ClientType.ACARDE), getConfig().getString("Config.Client.Host"), getConfig().getInt("Config.Client.Port"), (id == -1 ? "Test-Server" + Bukkit.getPort() : "a" + id));
			...
			if (!getConfig().getBoolean("Config.Server.World-Save"))
	...
	public void onDisable() {
		...
		for (GameType type : GameType.values())
			UtilFile.DeleteFolder(new File(type.getShortName().toLowerCase()));
