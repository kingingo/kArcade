...
public class CommandDetective implements CommandExecutor, Listener{
	...
	public boolean onCommand(CommandSender cs, Command cmd, String arg2,String[] args) {
		...
		if(!(t_p>0)){
			UtilPlayer.sendMessage(((Player)cs),Language.getText(((Player)cs), "PREFIX_GAME", TTT.getType().getTyp())+Language.getText(((Player)cs), "TTT_PASSE_KEINE","Detective"));
			return false;
		}
		...
		if(tt>=t){
			UtilPlayer.sendMessage(((Player)cs),Language.getText(((Player)cs), "PREFIX_GAME", TTT.getType().getTyp())+Language.getText(((Player)cs), "TTT_PASSE_MAX_USED","Detective"));
			return false;
		}
		t_p=t_p-1;
		TTT.getStats().setInt( ((Player)cs) , t_p, StatsKey.TTT_PAESSE);
		TTT.addTeam(((Player)cs) , Team.DETECTIVE);
		UtilPlayer.sendMessage(((Player)cs),Language.getText(((Player)cs), "PREFIX_GAME", TTT.getType().getTyp())+Language.getText(((Player)cs), "TTT_PASSE_USE",new String[]{"Detective",String.valueOf(t_p)}));
		return false;
	}
