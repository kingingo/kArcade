...
public class SurvivalGames extends TeamGame{
	...
	public void Start_Game(UpdateEvent ev){
		...
		setStart(getStart()-1);
		for(Player p : UtilServer.getPlayers())UtilDisplay.displayTextBar(p,TranslationHandler.getText(p, "GAME_START_IN", getStart()));
		switch(getStart()){
			case 20:
				setDamage(false);
...
//	 }
	...
	public void inGame(UpdateEvent ev){
		...
		setStart(getStart()-1);
		for(Player p : UtilServer.getPlayers())UtilDisplay.displayTextBar(p, TranslationHandler.getText(p, "DEATHMATCH_END_IN", getStart()));
		switch(getStart()){
			case 180:
