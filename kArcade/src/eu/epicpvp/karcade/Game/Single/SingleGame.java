...
public class SingleGame extends Game{
	...
	public void PlaceBlockInMap(BlockPlaceEvent ev){
		if(getManager().getPermManager().hasPermission(ev.getPlayer(), PermissionType.ALL_PERMISSION)||ev.getPlayer().isOp())return;
	...
	public void BreakBlockInMap(BlockBreakEvent ev){
		...
		if(getGameList().getPlayers(PlayerState.OUT).contains(ev.getPlayer()))ev.setCancelled(true);
		  if(getWorldData().getWorld() == null || (getWorldData().getWorld() != null && getWorldData().getWorld().getUID() != ev.getBlock().getWorld().getUID()))ev.setCancelled(true);
	...
	public void DropNNN(PlayerDeathEvent ev){
	 ...
	 public void JoinRanking(PlayerJoinEvent ev){
	  ...
	  public void Joinnow(PlayerJoinEvent ev){
		  ...
		  ev.getPlayer().sendMessage(Language.getText(ev.getPlayer(), "PREFIX")+"§eDu hast eine Map f§r uns gebaut? Melde sie im Forum und wir nehmen sie!§b http://EpicPvP.me/");
	  ...
	  public void Login(PlayerLoginEvent ev){
		 if(UtilServer.getPlayers().size()>=getMax_Players()&&isState(GameState.LobbyPhase)){
			  if(getManager().getPermManager().hasPermission(ev.getPlayer(), PermissionType.JOIN_FULL_SERVER)||getManager().getPermManager().hasPermission(ev.getPlayer(), PermissionType.JOIN_FULL_SERVER)){
				  boolean b = false;
	  ...
	  public void BreakBlockLobby(BlockBreakEvent ev){
		  ...
		  if(getWorldData().getWorld() == null)ev.setCancelled(true);
	  }
	  ...
	  @EventHandler
		public void Restart(UpdateEvent ev){
			...
			case 25:
				...
				break;
			case 23:getStats().saveAll(); break;
	  ...
	  @EventHandler(priority=EventPriority.HIGHEST)
		...
		public void Lobby(UpdateEvent ev){
			...
			if(start!=0){
				...
				case 5: Bukkit.getPluginManager().callEvent(new GamePreStartEvent(this)); break;
