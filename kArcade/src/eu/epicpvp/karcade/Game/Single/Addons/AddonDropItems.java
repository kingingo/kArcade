...
public class AddonDropItems extends kListener {
	...
	public void Update(UpdateEvent ev){
		...
		if(ev.getType()==UpdateType.SEC){
			for(Location loc : getInstance().getWorldData().getSpawnLocations(Team.BRONZE)) drop(loc,new ItemStack(Material.CLAY_BRICK),drop_rate);
		}
		
		if(ev.getType()==UpdateType.SLOW){
			if(getInstance().getWorldData().existLoc(Team.SILBER)){
				for(Location loc : getInstance().getWorldData().getSpawnLocations(Team.SILBER)) drop(loc,new ItemStack(Material.IRON_INGOT),drop_rate);
			}else{
				for(Location loc : getInstance().getWorldData().getSpawnLocations(Team.BRONZE)) drop(loc,new ItemStack(Material.IRON_INGOT),drop_rate);
			}
		}
		
		if(ev.getType()==UpdateType.SLOWER){
			if(getInstance().getWorldData().existLoc(Team.GOLD)){
				for(Location loc : getInstance().getWorldData().getSpawnLocations(Team.GOLD)) drop(loc,new ItemStack(Material.GOLD_INGOT),drop_rate, "§bGold");
			}else{
				for(Location loc : getInstance().getWorldData().getSpawnLocations(Team.BRONZE)) drop(loc,new ItemStack(Material.GOLD_INGOT),drop_rate);
			}
		}
	}
