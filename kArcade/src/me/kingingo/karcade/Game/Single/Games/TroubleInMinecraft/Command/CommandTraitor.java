...
public class CommandTraitor implements CommandExecutor, Listener{
	...
	public boolean onCommand(CommandSender cs, Command cmd, String arg2,String[] args) {
		...
		if(!(t_p>0)){
			UtilPlayer.sendMessage(((Player)cs),Text.PREFIX_GAME.getText(TTT.getType().getTyp())+ Text.TTT_PASSE_KEINE.getText("Traitor"));
			return false;
		}
		...
		if(tt>=t){
			((Player)cs).sendMessage(Text.PREFIX_GAME.getText(TTT.getType().getTyp())+ Text.TTT_PASSE_MAX_USED.getText("Traitor"));
			return false;
		}
		t_p=t_p-1;
		TTT.getStats().setInt( ((Player)cs) , t_p, Stats.TTT_PÄSSE);
		TTT.addTeam(((Player)cs) , Team.TRAITOR);
		UtilPlayer.sendMessage(((Player)cs),Text.PREFIX_GAME.getText(TTT.getType().getTyp())+ Text.TTT_PASSE_USE.getText(new String[]{"Traitor",String.valueOf(t_p)}));
		return false;
	}
