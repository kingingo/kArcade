...
public class AddonDropItems extends kListener {
	...
	@Getter
	private CaveWars instance;
	private int i=0;
	
	public AddonDropItems(CaveWars instance){
	...
	public void Update(UpdateEvent ev){
		...
		if(ev.getType()==UpdateType.SEC){
			for(Location loc : getInstance().getWorldData().getLocs(Team.WHITE)) drop(loc,new ItemStack(Material.CLAY_BRICK),1);
		}
		
		if(ev.getType()==UpdateType.SLOW){
			if(getInstance().getWorldData().existLoc(Team.PURPLE)){
				for(Location loc : getInstance().getWorldData().getLocs(Team.PURPLE)) drop(loc,new ItemStack(Material.IRON_INGOT),1);
			}else{
				for(Location loc : getInstance().getWorldData().getLocs(Team.WHITE)) drop(loc,new ItemStack(Material.IRON_INGOT),1);
			}
		}
		
		if(ev.getType()==UpdateType.SLOWER){
			if(getInstance().getWorldData().existLoc(Team.ORANGE)){
				for(Location loc : getInstance().getWorldData().getLocs(Team.ORANGE)) drop(loc,new ItemStack(Material.GOLD_INGOT),1, "§bGold");
			}else{
				for(Location loc : getInstance().getWorldData().getLocs(Team.WHITE)) drop(loc,new ItemStack(Material.GOLD_INGOT),1);
			}
		}
	}
